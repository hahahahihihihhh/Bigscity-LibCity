...
class TrafficDynamicDataset(TrafficStateDataset):
    """
    交通状态预测数据集的基类。
    ...
    def _generate_input_data(self, df):
        ...
        for t in range(min_t, max_t):
            ...
            x_time = (t + x_offsets).astype(np.float32)
            y_time = (t + y_offsets).astype(np.float32)

            spatial_shape = x_t.shape[1:-1]  # "..." 对应的维度
            x_time_full = x_time.reshape((-1,) + (1,) * len(spatial_shape) + (1,))
            y_time_full = y_time.reshape((-1,) + (1,) * len(spatial_shape) + (1,))

            x_time_full = np.broadcast_to(x_time_full, x_t.shape[:-1] + (1,))
            y_time_full = np.broadcast_to(y_time_full, y_t.shape[:-1] + (1,))

            x_t = np.concatenate([x_t, x_time_full], axis=-1)
            y_t = np.concatenate([y_t, y_time_full], axis=-1)
