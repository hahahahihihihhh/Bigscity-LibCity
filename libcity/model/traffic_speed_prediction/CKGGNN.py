...
class EncoderModel_goal(nn.Module, Seq2SeqAttrs):
    def __init__(self, config, adj_mx):
        nn.Module.__init__(self)
        Seq2SeqAttrs.__init__(self, config, adj_mx)
        self.input_dim = config.get("feature_dim", 1)   # 1
        self.dcgru_layers = nn.ModuleList()
        self.dcgru_layers.append(DCGRUCell(self.input_dim, self.rnn_units, adj_mx, self.max_diffusion_step,
                                           self.num_nodes, self.device, filter_type=self.filter_type))
        for i in range(1, self.num_rnn_layers):
...
class CKGGNN(AbstractTrafficStateModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature):
        ...
        self.base_feature_dim = data_feature.get('feature_dim', 1)
        ...
        # feature dim
        self.pure_feature_dim = self.base_feature_dim
        ...
        self.feature_dim = self.ctx_feature_dim + self.pure_feature_dim

        config['num_nodes'] = self.num_nodes
        config['feature_dim'] = self.feature_dim
        self.output_dim = self.base_feature_dim
    ...
    def obtain_spat_kge_final(self, x_goal, x_auxi, dict_kge):
        ...
        # sub_kg for spat
        subdict_spat = dict_kge['sub_spat']  # {area_id: {area/poi/land/link: [fact]}}
        ...
        for _dim3 in range(x_goal.shape[2]):  # num_nodes
            area_str = 'area_' + str(int(x_auxi[0, 0, _dim3, 3]))
            ...
            embed2_area = np.zeros(self.embed_dim, dtype=np.float)
            ...
            _subkg_fact_area, _subkg_fact_poi, _subkg_fact_land = [], [], []
            if 'area' in spat_attr_used_list:
                _subkg_fact_area.extend(subdict_spat[area_str]['area'])
            if 'poi' in spat_attr_used_list:
                _subkg_fact_poi.extend(subdict_spat[area_str]['poi'])
            if 'land' in spat_attr_used_list:
                _subkg_fact_land.extend(subdict_spat[area_str]['land'])
            # embed 1
            for _fact in _subkg_fact_area:
                if _fact[0] == area_str and _fact[1] == 'self_ffspeed':  # embed-1
            ...
            # embed 2
            for _fact in _subkg_fact_area:  # area, [area_id, intersectWithArea[Order], area_id]
                _weight = float(_fact[3])
                if self.kg_context == 'temp': _weight = 0.0
                if _fact[0] == area_str:
                    if _fact[1] != 'self_ffspeed':
                        embed2_area += self.cal_ent_via_rel(self.spat_model_used, spat_ent_kge[spat_ent_label[_fact[2]], :],
                                                            spat_rel_kge[spat_rel_label[_fact[1]], :], _weight)
                else:
                    self._logger.info('[ERROR-area]One fact doesnot find its area_id with fact.{}'.format(_fact))
                    exit()
            for _fact in _subkg_fact_poi:  # poi, [poi_type, locateInBuffer[Dist], area_id]
                _weight = float(_fact[3])
                if self.kg_context == 'temp': _weight = 0.0
                if _fact[2] == area_str:
                    ...
                    self._logger.info('[ERROR-poi]One fact doesnot find its area_id with fact.{}'.format(_fact))
                    exit()
            for _fact in _subkg_fact_land:  # land, [land_type, intersectWithBuffer[Dist], area_id]
                _weight = float(_fact[3])
                if self.kg_context == 'temp': _weight = 0.0
                if _fact[2] == area_str:
                    ...
                    self._logger.info('[ERROR-land]One fact doesnot find its area_id with fact.{}'.format(_fact))
                    exit()
            spat_embed_2[: self.embed_dim] = embed2_area
            ...
            if self.spat_link_attr > 0:
                for _fact in subdict_spat[area_str]['link']:  # link, [area_id, spatiallyLinkDegree[Degree], area_id]
                    _weight = float(_fact[3])
                    if self.kg_context == 'temp': _weight = 0.0
                    if _fact[0] == area_str:
                        ...
                        self._logger.info('[ERROR-link]One fact doesnot find its area_id with fact.{}'.format(_fact))
                        exit()
            final_embed = np.concatenate((spat_embed_1, spat_embed_2, spat_embed_3), axis=0)
            subdict_spat_kge[area_str] = final_embed
            ...
            for _dim1 in range(x_goal.shape[0]):  # batch_size
                if _dim1 == 0:
                    for _dim2 in range(x_goal.shape[1]):  # input_window
                        ...
                        area_strs = ['area_' + str(int(i)) for i in x_auxi[_dim1, _dim2, :, 3]]
                        x_spat[_dim1, _dim2, :, :] = [subdict_spat_kge[area] for area in area_strs]
                        x_temp[_dim1, _dim2, :, :] = [subdict_temp_kge[area] for area in area_strs]
                else:
                    ...
                    area_strs = ['area_' + str(int(i)) for i in x_auxi[_dim1, new_dim2, :, 3]]
                    x_spat[_dim1, -1, :, :] = [subdict_spat_kge[area] for area in area_strs]
                    x_temp[_dim1, -1, :, :] = [subdict_temp_kge[area] for area in area_strs]
        else:  # self.kg_context == 'spat'
            area_strs = ['area_' + str(int(i)) for i in x_auxi[0, 0, :, 3]]
            common_x_spat_value = [subdict_spat_kge[area] for area in area_strs]  # x_spat[0, 0, :, :]
