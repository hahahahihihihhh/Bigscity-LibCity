...
class TGCNCell(nn.Module):
    def __init__(self, num_units, adj_mx, num_nodes, ke_dim, device, dataset, input_dim=1):
        # ----------------------初始化参数---------------------------#
        super().__init__()
    ...
    def init_params(self, bias_start=0.0):
        ...
        weight_unit = torch.nn.Parameter(torch.empty((self.ke_dim, self.num_nodes), device=self._device))
        bias_unit = torch.nn.Parameter(torch.empty(self.num_nodes, device=self._device))

        torch.nn.init.xavier_normal_(weight_0)
        torch.nn.init.xavier_normal_(weight_1)
        torch.nn.init.xavier_normal_(weight_unit)
        torch.nn.init.constant_(bias_0, bias_start)
        torch.nn.init.constant_(bias_1, bias_start)
        torch.nn.init.constant_(bias_unit, bias_start)

        self.register_parameter(name='weights_0', param=weight_0)
        self.register_parameter(name='weights_1', param=weight_1)
        self.register_parameter(name='weight_unit', param=weight_unit)
        self.register_parameter(name='bias_0', param=bias_0)
        self.register_parameter(name='bias_1', param=bias_1)
        self.register_parameter(name='bias_unit', param=bias_unit)
        # print(weight_0.shape, weight_1.shape, weight_unit.shape, bias_0.shape, bias_1.shape, bias_unit.shape)
        self.weights = {weight_0.shape: weight_0, weight_1.shape: weight_1, weight_unit.shape: weight_unit}
        self.biases = {bias_0.shape: bias_0, bias_1.shape: bias_1, bias_unit.shape: bias_unit}
    ...
    def _gc(self, inputs, state, output_size, bias_start=0.0):
        ...
        """
        kgembedding = pd.read_csv('./kg_assist/{}/KST_GCN/kg_embedding_d{}.csv'.format(self.dataset, self.ke_dim)
                                  , header=None)
        ...
        weights = self.weights[(input_size, output_size)]
...
class KST_GCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.ke_dim = config.get('ke_dim', 20)

        # -------------------构造模型-----------------------------
        self.tgcn_model = TGCNCell(self.gru_units, self.adj_mx, self.num_nodes, self.ke_dim, self.device, self.dataset, self.input_dim)
