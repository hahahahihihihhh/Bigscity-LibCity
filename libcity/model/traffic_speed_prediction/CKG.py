...
class CKGGNN(AbstractTrafficStateModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature):
        ...
        # 原始输入特征维度（例如：1=single / 2=inflow&outflow）
        ...
        # feature dim
        # 纯交通特征维度（不含KG上下文），应与数据集 feature_dim 保持一致
        ...
        # Encoder 输入维度 = KG上下文维度 + 纯交通特征维度
        self.feature_dim = self.ctx_feature_dim + self.pure_feature_dim

        config['num_nodes'] = self.num_nodes
        config['feature_dim'] = self.feature_dim
        # Decoder 输入/输出维度（预测维度），默认与 base_feature_dim 一致

        # output dim (usually equals base_feature_dim for multi-variate targets like inflow/outflow)
        self.output_dim = config.get('output_dim', data_feature.get('output_dim', self.base_feature_dim))
        if self.output_dim == 1 and self.base_feature_dim > 1:
            # If the dataset exposes multiple features but output_dim is left as 1, default to predicting all features.
