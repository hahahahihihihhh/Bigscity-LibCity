...
class MCK_GWN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.ke_dim = config.get('ke_dim', 256)
        self.max_hop = config.get('max_hop', 2)
        self.sparsity = config.get('sparsity', 3)
        self.input_dim = config.get('input_dim', 1)
        self.use_mh_adj = config.get('use_mh_adj', True)
        ...
        self.start_conv = nn.Conv2d(in_channels=self.feature_dim,       # model_dim feature_dim input_dim
        ...
        # !!!
        if (self.use_mh_adj):
            att_mh_score = pd.read_csv(os.path.join("./kg_assist", "{}/MCK_GWN/poi_STF_d{}_hop{}_s{}.csv".
                                                    format(self.dataset, self.ke_dim, self.max_hop, self.sparsity)), header=None).values
            # att_mh_score = pd.read_csv(os.path.join("./kg_assist", "{}/MCK_GWN/{}_STF3.csv".
            #                                         format(self.dataset, self.ke_dim)),
            #                            header=None).values
            att_mh_score = torch.from_numpy(att_mh_score.astype(np.float32)).to(self.device)
            self.supports += [att_mh_score]
            self.supports_len += 1
            self._logger.info('add attributes enhanced multi-hop score in forward process')
        # !!!
        # mh_score_rel_att = np.loadtxt(os.path.join("./kg_assist/{}/MCK_GWN".
        #                                    format(self.dataset),
        #                                    "{}_{}hop_rel_att.csv".
        #                                    format(self.ke_dim, self.max_hop)))
        # mh_score_rel_att = torch.from_numpy(mh_score_rel_att.astype(np.float32)).to(self.device)
        # self.supports += [mh_score_rel_att]
        # self.supports_len += 1
        # self._logger.info('add attributes enhanced multi hop score in forward process')
        #
