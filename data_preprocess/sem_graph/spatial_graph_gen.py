...
import numpy as np
import pandas as pd
from libcity.data_preprocess.utils.utils import load_dict, toTxt, initialize_seed, ensure_dir
    ...
    for _i in range(num_node):
        for _j in range(num_node):
            ...
            if _i != _j and dis[_i][_j] != 0 and dis[_i][_j] <= link_cap:
                lst.append((id2road[_i], "adj_spat", id2road[_j]))
                w[_i, _j] = 1 * (decay_ratio ** (dis[_i][_j] - 1))
    sg_prefix = os.path.join(dataset, "spatial_graph")
    ensure_dir(sg_prefix)
    w_sg_path = os.path.join(sg_prefix, 'w_sg.csv')
    pd.DataFrame(w).to_csv(w_sg_path, index=False, header=None)
    print("The weight matrix of spatial graph is generated!")
    tri_sg_path = os.path.join(sg_prefix, 'tri_sg.txt')
